...
/*
        ...
        return value: on success =>  the number of bytes read
                      if timeout expires => READ_TIMEOUT
                                          if our partner closes connection =>
   READ_CLOSECONN if a read error occurs => READ_ERROR if a select error occurs
   => READ_SELECT_ERROR Read does never modifies timeout value
 */
